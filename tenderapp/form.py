...
class InvoiceModelForm(forms.ModelForm):
    class Meta:
        model = Invoice_Model
        PO_Issued = forms.CheckboxInput(attrs={})
        fields = ('invoice_for','total_gst','vendor','project','due_date','rml','invoice_date','po_no','PO_Issued','total_amount','total_igst','grand_total','terms','total_sgst','total_cgst','remark',)
        widgets = {
            'invoice_for': forms.Select(choices=invoice_choices,attrs={
                'class': 'form-control round-form',
                'placeholder': 'Enter Invoice here',
