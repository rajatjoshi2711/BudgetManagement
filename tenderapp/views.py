...
from . import models
from .form import PropertyDetails, SaleSignUpForm, FinanceSignUpForm,InvoiceModelForm, PurchaseModelForm,InvoiceFormset,ParticularsFormset,ManagementSignUpForm, PurchaseSignUpForm,Vendor_Form,Update_Vendor_Form
...
from .models import Vendor_Model,Payment_Model,Request_model,Project_model,Invoice_Model, Rml_model
...
def purchase_order(request):
    ...
    item = models.Project_model.objects.all()
    ...
    if request.method == 'GET':
        ...
        if purchaseform.is_valid() and formset.is_valid():
            ...
            print("rml_no:",rml_no)
            print("rml_available_budget:",rml_available_budget[0].available)
            available_budget = rml_available_budget[0].available
            if int(request.POST.get("amount")) <= rml_available_budget[0].available:
                print("new available rml amount:",rml_available_budget[0].available - int(request.POST.get("amount")))
                models.Rml_model.objects.filter(rml_no=rml_no).update(available=rml_available_budget[0].available - int(request.POST.get("amount")))
                ...
                messages.warning(request, 'Taxable amount is more than available budget for RML. Please modify the details and submit again.')
...
def new_invoice(request):
    ...
    if request.method == 'GET':
        ...
        if purchaseform.is_valid() and formset.is_valid():
            amount_in_invoice = request.POST.get("total_amount")
            if(request.POST.get("PO_Issued")):
                print("In PO Issued")
                PO_in_invoice = request.POST.get("po_no")
                PO_taxable_amount = models.Purchase_Order_Model.objects.all().filter(id=PO_in_invoice)[0].sub_total
                if PO_taxable_amount >= int(amount_in_invoice):
                    ...
                    messages.warning(request, 'PO Taxable amount is less than the amount in invoice. Please modify the details and submit again.')
                ...
                print("rml_available_budget:",rml_available_budget[0].available)
                available_budget = rml_available_budget[0].available
                if int(request.POST.get("total_amount")) <= rml_available_budget[0].available:
                    print("new available rml amount:",rml_available_budget[0].available - int(request.POST.get("total_amount")))
                    models.Rml_model.objects.filter(rml_no=rml_no).update(available=rml_available_budget[0].available - int(request.POST.get("total_amount")))
                    invo = purchaseform.save(commit=False)
                    invo.user = request.user
                    invo.save()
                    for form in formset:
                    ...
                    messages.warning(request, 'PO Taxable amount is less than the amount in invoice. Please modify the details and submit again.')
                    return redirect('new_invoice')
            # print("Amount in invoice: ", amount_in_invoice)
            # print("PO in invoice", PO_in_invoice)
            # print("PO taxable amount", PO_taxable_amount)
            # A condition here to check whether PO has been issued for this invoice or not           
...
def new_advance_payment(request):
    ...
    if request.method == "POST":
        PO_Issued= request.POST.get('PO_Issued')
        po_id = request.POST.get('po_id', 0)
        ...
        rml_no= request.POST.get('rml_no')
        ab = Purchase_Order_Model.objects.get(pk=po_id)
        project_no = int(str(ab.project))
        print(project_no)        
        if(request.POST.get("PO_Issued")):
            PO_in_invoice = request.POST.get("po_id")
            PO_taxable_amount = models.Purchase_Order_Model.objects.all().filter(id=PO_in_invoice)[0].sub_total
            if PO_taxable_amount >= int(amount):
                item = Request_model(po_id=po_id, invoice_id=invoice_id, vendor=vendor, amount=amount, payment_terms=pay_term,
                          due_date=due_date, project_no=project_no)
                item.save()
                return view_advance_payment(request)
            else:
                return  new_advance_payment(request)
        else:           
            item = Request_model(po_id=po_id, invoice_id=invoice_id, vendor=vendor, amount=amount, payment_terms=pay_term,
                    due_date=due_date,  project_no=project_no)
            rml_available_budget = models.Rml_model.objects.all().filter(rml_no=rml_no)
            ...
            available_budget = rml_available_budget[0].available
            if int(request.POST.get("amount")) <= rml_available_budget[0].available:                
                item = Request_model(po_id="000", invoice_id=invoice_id, vendor=vendor, amount=amount, payment_terms=pay_term,
                          due_date=due_date, project_no=project_no)
                item.save()
                return view_advance_payment(request)
            else:
    ...
    return render(request, 'new_advance_payment.html', {'item': item})